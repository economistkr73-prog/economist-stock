 ...
 *
 *  cafe24 cron 등록 예 (URL 호출):
 *    ★권장(저위험·분산) :  *\/10 3-8 1 * *
 *        /cron_naver_collect.php?key=econ-naver-9x2k&max=8&delay=10
 *      → 매월 1일 03:00~08:50, 10분 간격 36회. 회당 8지역×10초딜레이(≈80초).
 *        8×36=288 ≥ 229 시군구 커버, 다 done 되면 남은 fire 는 no-op.
 *      ※ 네이버 안티봇은 IP당 누적 예산형 → 호출 사이 10분 휴식으로 429 회피.
 *        몰아치기(연속 호출·짧은 휴식)는 IP 일시차단 위험.
 *    (구) 분단위 10회 :  *\/6 3 1 * *   ?key=econ-naver-9x2k
 *      → 6분 간격 10회·회당 25지역(3초). 빠르나 누적 요청 과다로 429 트립 위험.
 *
 *  파라미터:
 *    key       (필수) 실행 토큰
