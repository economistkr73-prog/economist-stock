...
| 3 | **평일** 08:05·**17:05** `5 8,17 * * 1-5` | 다트 상장주식 재무보고서 | `cron/dart_collect.php` `job=fresh&bg=1` | 0.07s | ✅ **2026-07-30 bg=1 적용** · ⏳**2026-08-11 오후 17:05 fire 추가 — 크론 사이트 EDIT 에서 시간 8→8,17 로 바꿔야 한다.** DART 재무 API 가 공시 접수보다 늦어(실측 08:05 반기 46종목 → 17:00 66종목) 실적 알림이 다다음 날로 밀리는 종목을 오후 fire 가 따라잡는다. 알림 중복은 `pf_alert_log` 가 막는다 |
...
#17 ?task=boxbrk&k=econ-cron-j7k2            ← 신설(평일 16:20 · 박스 상향돌파 · 2026-08-11 등록)
#18 ?task=agoda&k=econ-cron-j7k2             ← 신설(매일 08:40 · 아고다 호텔 가격 · 2026-08-20 — 사이트 등록 필요)
```
...
- **#17(박스 상향돌파)은 #9 와 같은 16:20 이지만 무해하다** — 적재·판정은 API 0콜 수 초이고, 분봉만
  키움 콜(1 req/s · 최대 400초 → 16:27 종료)이라 #9(네이버)와 API 도 표도 다르다. **#16(16:45 키움)과도
  안 겹친다.** 16:20 인 이유는 #8(15:50 `dart_eod`)이 오늘 행을 넣은 «뒤»여야 하기 때문 — 앞으로 당기면
  매일 「그 날 봉이 없다」로 조용히 0건이 된다.
